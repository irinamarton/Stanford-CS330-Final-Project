...
def get_categorical_tickers():
    ...
    ticker_dict = {}
    all_tickers = []
    ticker_dict['train'] = ['AEP', 'CEG', 'EVRG', 'EXC', 'APA', 'CHKEW', 'FANG', 'AXON', 'BKR', 'CSX', 'DASH', 'FOXA',
                            'FWONA', 'HON', 'JBHT', 'LECO', 'LIN', 'NDSN', 'ODFL', 'PARAA', 'ROP', 'CCEP', 'CELH',
                            'KDP', 'KHC', 'MDLZ', 'MNST', 'AGNCN', 'EQIX', 'GLPI', 'HST', 'DKNG', 'DLTR', 'EBAY',
                            'EXPD', 'EXPE', 'FAST', 'GRAB', 'HTHT', 'JD', 'LI', 'LKQ', 'LULU', 'MAR', 'MELI', 'NFLX',
                            'NWSA', 'ORLY', 'PAYX', 'PCAR', 'PDD', 'POOL', 'PYPL', 'QRTEP', 'RIVN', 'ROST', 'RYAAY',
                            'SBUX', 'SIRI', 'TCOM', 'TSCO', 'TSLA', 'UAL', 'ULTA', 'VRSK', 'WMG', 'CHTR', 'CMCSA',
                            'CSCO', 'LBRDA', 'ROKU', 'TMUS', 'CHKP', 'CRWD', 'CTSH', 'DDOG', 'EA', 'ENPH', 'ENTG',
                            'ERIC', 'FLEX', 'FSLR', 'FTNT', 'GEN', 'GFS', 'GOOGL', 'INTC', 'INTU', 'JKHY', 'KLAC',
                            'LOGI', 'LRCX', 'MANH', 'MCHP', 'MDB', 'META', 'MPWR', 'MRVL', 'MSFT', 'MU', 'NICE', 'NTAP',
                            'NTES', 'NVDA', 'NXPI', 'OKTA', 'ON', 'PANW', 'PTC', 'QCOM', 'SMCI', 'SNPS', 'SPLK', 'SSNC',
                            'STX', 'SWKS', 'TEAM', 'TTD', 'TTWO', 'TXN', 'VRSN', 'VRTX', 'WDAY', 'WDC', 'ZBRA', 'ZM',
                            'ZS', 'CME', 'COIN', 'CSGP', 'ERIE', 'FCNCA', 'FITB', 'HBAN', 'LPLA', 'MORN', 'NDAQ',
                            'NTRS', 'PFG', 'SLMBP', 'TROW', 'TW', 'VLYPO', 'WTW', 'XP', 'ALGN', 'ALNY', 'AMGN', 'ARGX',
                            'AZN', 'BGNE', 'BIIB', 'BMRN', 'BNTX', 'COO', 'DXCM', 'EXAS', 'GILD', 'HOLX', 'ICLR',
                            'IDXX',
                            'ILMN', 'INCY', 'ISRG', 'LEGN', 'MRNA', 'NBIX', 'PODD']
    ticker_dict['val'] = ['LNT', 'XEL', 'PAA', 'SAIA', 'STLD', 'TER', 'TRMB', 'PEP', 'WBA', 'REG', 'SBAC', 'ABNB',
                          'AKAM', 'AMZN', 'BKNG', 'CASY', 'CDW', 'COST', 'CPRT', 'CTAS', 'VOD', 'WBD', 'ADBE', 'ADI',
                          'ADP', 'ADSK', 'AMAT', 'AMD', 'ANSS', 'APP', 'ASML', 'AVGO', 'AZPN', 'BIDU', 'BSY', 'CDNS',
                          'ACGL', 'ARCC', 'BPYPP', 'CG', 'CINF', 'REGN', 'RPRX', 'SGEN', 'SNY', 'UTHR', 'VTRS']
...
def data_w_context(data, target):
    ...
    for i in range(len(data)-1):
        data_i = [arr.item() for arr in data[i]]
        data_i.append(target[i])
        data_i2 = [arr.item() for arr in data[i+1]]
        x = np.concatenate((data_i, data_i2), axis=0)
        context_data.append(torch.tensor(x, dtype=torch.float32).reshape(21, 1))
    context_target = target[1:]
    #context_data = torch.stack(context_data)
    #print("SHAPe", context_target.shape, np.array(context_data).shape)
